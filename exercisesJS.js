...
function problem1() {
    ...
    return problem1_input.replace(/[iIl|!/]/g, 1).replace(/[oO@Θθ]/g, "0")
}
...
function problem2() {
    ...
    //split input at spaces to get each byte, match the first "1" and get the index and add 1, map this to an array and then join
    return input.split(/\s/).map(str => str.match(/1/)['index'] + 1).join(" ")
}
...
function problem3() {
    ...
    //split input at spaces to get each byte, search each byte to get the index of the last "1" followed by 0 or more "0" that end the byte
    //map the index to an array and join
    return input.split(/\s/).map(str => str.search(/10*$/) + 1).join(" ")
}
...
function problem5() {
    ...
    let input = '00100001 01001100 10100011 11010010 11011011 01010000 10001001 00110110 00100001 11001010 11010001 10111001 10111001 00000110 11000110 10100000 11010100 10000100 00010000 00011110 00111011 11011001 11011110 11010100 01000000 10101011 10000100 01110111 11110110 10011100 11111100 01001101 10000011 11010000 10011111 01101101 01101100 01101011 01011001 11110010 11010011 00101001 01111101 01011110 10010111 01100110 10011101 10101001 11010011 11011010 11111111 00111100 10100000 11110100 10011111 00101101 01011101 00100100 00000010 10011011 10010111 01000010 10001110 00011010 10010111 00100101 11100000 00101110 11010001 10100101 10010111 01100111 00101010 11010011 10001110 01100011 01000111 11001101 01100100 01111111 11001011 11101010 01001110 11101111 00010101 01100001 11001110 00001010 10101111 01101100 01100001 01110111 11100101 10010111 01011101 00101110 10011101 11011111 10110000 01000101'
    input = '01000011 10011110 11101011 11000000'
    let solution = ''
    ...
    //split at whitespace to get each byte
    //match bytes starting with 0 or 1
        //for bytes starting with 1, capture everything before and after LSB

    //solution = input.split(/\s/).map(byte => byte.match(/(?<pre>^1[01]*)1(?<post>0*)$)/))
    //solution = solution.groups.pre.replaceAll(/1/g, "x").replaceAll(/0/g, '1').replaceAll(/x/g, '0')

    //solution = solution.map(str => str.match(/(?<pre>[01]*)1(?<post>0*)$/))
    //solution = solution.map(a => a.groups.pre.replaceAll(/1/g, "x").replaceAll(/0/g, '1').replaceAll(/x/g, '0') + 1 + a.groups.post)
    //console.log(solution.map(a => a.groups.post))
    //solution = solution
